...
class MainTopic(Topic):
    ...
    @cached_property
    def sub_topic_var_names(self) -> Generator[str]:
        for sub_topic_var_name in self._extract_sub_topic_var_names_from_source():
            yield sub_topic_var_name
    
    def _extract_sub_topic_var_names_from_source(self) -> Generator[str]:
        wrapped = self.wrapped
        for sub_topic_var_name in wrapped.__code__.co_varnames:
            if not sub_topic_var_name.isupper():
                continue
            sub_topic_name = sub_topic_var_name.strip().lower()[1:]
            yield sub_topic_name
    
    def print_unused_subtopics(self):
        """python -m termwiki <MAIN TOPIC> --doctor calls this"""
        wrapped = self.wrapped
